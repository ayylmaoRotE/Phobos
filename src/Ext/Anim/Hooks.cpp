...
// tiny helpers (keeps call sites tight; no state)
namespace _AnimPerf
{
...
DEFINE_HOOK(0x423B95, AnimClass_AI_HideIfNoOre_Threshold, 0x8)
{
	...
	const int threshold = std::abs(_AnimPerf::TypeExt(pType)->HideIfNoOre_Threshold.Get());
...
// Remove Ares' old anim damage site
DEFINE_PATCH(0x424538, 0x8B, 0x8E, 0xCC, 0x00, 0x00, 0x00);

// New, guarded damage logic (deterministic)
DEFINE_HOOK(0x42453E, AnimClass_AI_Damage, 0x6)
{
	...
	else if (delay <= 0 || base < 1.0)
	{
		...
		if (total >= 1.0)
		{
			...
			pThis->Accum = total - applied; // keep remainder
		}
		else
		{
			pThis->Accum = total;
	...
	// separate fallback to owner via owner object / parent building
	if (!pOwner)
	{
		if (pOwnerObj)
	...
	else
	{
		...
		if (!pWH)
		{
			pWH = std::strcmp(pType->get_ID(), "INVISO")
...
DEFINE_HOOK(0x42465D, AnimClass_AI_NullTypeCheck, 0x6)
{
	...
	if (!pThis->Type)
	{
		...
		const auto map = pThis->GetMapCoords();
...
DEFINE_HOOK(0x4242E1, AnimClass_AI_TrailerAnim, 0x5)
{
	...
	auto* const pType = pThis->Type;
	if (!pType) return SkipGameCode;

	AnimTypeClass* const pTrailerType = pType->TrailerAnim;
	if (!pTrailerType) return SkipGameCode;

	const auto coords = pThis->GetCoords();
	if (auto* const pTrailerAnim = GameCreate<AnimClass>(pTrailerType, coords, 1, 1))
	{
...
// debris anims: deferred particle system attach
...
DEFINE_HOOK(0x423CC7, AnimClass_AI_HasExtras_Expired, 0x6)
{
	...
	if (!pThis || !pThis->Type) return SkipGameCode;
...
{
	GET(AnimClass*, pThis, ESI);
	auto* const pType = pThis->Type; if (!pType) return 0;
	auto* const pTypeExt = _AnimPerf::TypeExt(pType);
...
// RING1 alt path + normal path XDrawOffset
...
DEFINE_HOOK(0x423061, AnimClass_DrawIt_Visibility, 0x6)
{
	...
	HouseClass* pCurrentHouse = HouseClass::CurrentPlayer;
